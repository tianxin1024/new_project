...
if __name__ == "__main__":
    config = json.load(open("./config_HSIT_PRE.json"))
    print(config)
    print("------------------------------")
    model = HyperTransformerPre(config)
    print(model)

    # 定义参数
    batch_size = 1
    ms_channels = 102  # 高光谱通道数
    pan_size = 160     # 全色图像大小
    ms_size = 40       # 多光谱图像大小
    factor = 4         # 上采样因子

    X_MS  = torch.rand(batch_size, ms_channels, ms_size, ms_size)  # [1, 102, 40, 40]
    X_PAN = torch.rand(batch_size, pan_size, pan_size)            # [1, 160, 160]

    # 将模型和数据移到GPU（如果可用）
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    MS_image = X_MS.to(device)
    PAN_image = X_PAN.to(device)

    out = model(MS_image, PAN_image)['pred']
